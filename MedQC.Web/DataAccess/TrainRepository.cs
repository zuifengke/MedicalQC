...
{
    public class TrainRepository : BaseRepository<Models.Train>
    {
        ...
        /// <returns></returns>
        public IEnumerable<Train> LoadPageList(string title, int categoryID,string categoryCode, int startNum, int pageSize, out int rowCount)
        {
            rowCount = 0;
            var result = from p in db.Set<Train>()
            ...
            return result.ToList().Select(m => new Train()
            {
                ID = m.ID,
        ...
        /// <param name="rowCount">总个数</param>
        /// <returns></returns>
        public IEnumerable<Train> GetTrains(string categoryCode, string order)
        {
            var result = from p in db.Set<Train>()
                         join a in db.Set<Category>() on new { CategoryID = p.CategoryID } equals new { CategoryID = a.ID } into a_into
                         from a in a_into.DefaultIfEmpty()
