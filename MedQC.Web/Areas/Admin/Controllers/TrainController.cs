...
namespace MedQC.Web.Areas.Admin.Controllers
{
    ...
    public class TrainController : Controller
    {
        ...
        public ActionResult QueryData()
        {
            ...
            int totalCount = 0;
            var result = EnterRepository.GetRepositoryEnter().TrainRepository.LoadPageList(title, 0, categoryCode, (page - 1) * size, size, out totalCount).ToList();

            JsonHelper json = new JsonHelper();
            string strJson = string.Empty;
            foreach (Train item in result)
            {
        ...
        public ActionResult GetDetail()
        {
            ...
            var result = EnterRepository.GetRepositoryEnter().TrainRepository.LoadEntities(m => m.ID == id).FirstOrDefault();
            return Json(result);
        }
        ...
        public ActionResult Edit(FormCollection form)
        {
            ...
            item.ImagePath = string.IsNullOrEmpty(form["ImagePath"])?SystemContext.Instance.GetDefaultImg(): form["ImagePath"];
            item.Summary = HtmlContentHelper.GetSummary(item.Content);
            
            EnterRepository.GetRepositoryEnter().TrainRepository.Get(m => m.ID == item.ID);
            bool result = EnterRepository.GetRepositoryEnter().TrainRepository.EditEntity(item, new string[] { "Title", "Keywords", "CategoryID", "Content", "ModifyID", "ModifyTime", "ImagePath", "Summary"});
        ...
        public ActionResult Add(FormCollection form)
        {
            ...
            EnterRepository.GetRepositoryEnter().TrainRepository.AddEntity(item);
            if (EnterRepository.GetRepositoryEnter().SaveChange() > 0)
            {
            ...
            try
            {
                ...
                if (selectID != string.Empty && selectID != "0")
                {
                    ...
                    foreach (var item in ids)
                    {
                        ...
                        var train = EnterRepository.GetRepositoryEnter().TrainRepository.LoadEntities(m => m.ID == id).FirstOrDefault();
                        if (train.ArticleID == 0)
                        {
                            ...
                            EnterRepository.GetRepositoryEnter().TrainRepository.Get(m => m.ID == id);
                            EnterRepository.GetRepositoryEnter().TrainRepository.EditEntity(train, new string[] {"ArticleID" });
                            if (EnterRepository.GetRepositoryEnter().SaveChange() <= 0)
                            {
                                writeMsg = string.Format("同步失败");
                                break;
        ...
        public ActionResult Delete()
        {
            ...
            try
            {
                ...
                if (selectID != string.Empty && selectID != "0")
                {
                    ...
                    {
                        int id = int.Parse(item);
                        EnterRepository.GetRepositoryEnter().TrainRepository.DeleteEntity(new Train() { ID = id });
                    }
