...
namespace MedQC.Web.Areas.weixin.Controllers
{
    /// <summary>
    /// 微信授权账号信息统一处理控制
    /// </summary>
    public class AccountController : Controller
    {
        ...
        public ActionResult UserInfoCallback(string code, string state)
        {
            ...
            try
            {
                ...
                {
                    //绑定手机号，查找是否有相同号码的member,将memberID赋值给oauthuser，如果没有则创建member账号
                    return Redirect("/member/weixinregister?ID=" + oauthuser.ID.ToString());
                }
                WebCookieHelper.SetUserCookie(oauthuser.MemberID, oauthuser.Name, oauthuser.Tel, string.Empty, string.Empty, 7);
