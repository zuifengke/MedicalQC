...
namespace MedQC.Web.Areas.SysConfig.Controllers
{
    ...
    public class SysParameterController : Controller
    {
        ...
        {
            SysParameter sysparameter = new SysParameter() { ParentID = 0 };
            var lstMenus = SysParameterDao.Instance.GetSysParameters(sysparameter).ToList();
            ...
            dataRow["ParentID"] = "-1";
            dataRow["Name"] = "父节点";
            string strJson = JsonHelper.GetTreeJsonByTable(dtMenus, "ID", "Name", "ParentID", "-1", "");
        ...
        {
            int id = Request.Form["id"] != "" ? int.Parse(Request.Form["id"]) : 0;
            var item = SysParameterDao.Instance.QueryOne(id);
            return Json(item);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult Insert(SysParameter item)
        {
            ...
            bool result = SysParameterDao.Instance.Insert(item);
            if (result)
            {
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult Update(SysParameter item)
        {
            string writeMsg = string.Empty;
            //SysRole item = new SysRole();
            ...
            //item.Name = Request.Form["Name"] != "" ? Request.Form["Name"] : "";
            //item.Description = Request.Form["Description"] != "" ? Request.Form["Description"] : "";
            bool result = SysParameterDao.Instance.Update(item);
            if (result)
            {
                writeMsg = "{msg:\"保存成功!\",id:" + item.ID.ToString() + "}";
            }
        ...
        public ActionResult Delete()
        {
            ...
            if (ids != string.Empty && ids != "0")
            {
                ...
                foreach (var item in arrids)
                {
                    ...
                    bool result = SysParameterDao.Instance.Delete(id);
                    if (!result)
                    {
                        result = false;
                        break;
