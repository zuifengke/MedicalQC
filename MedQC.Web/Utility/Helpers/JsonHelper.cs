...

{
    ...
    public class JsonHelper
    {
        ...
        /// <param name="szCheckItems">选中的ID号集合</param>
        /// <returns></returns>
        public static string GetQcCheckPointTreeJson(List<Models.Meddoc.QcCheckPoint> checkPoints, int parentID)
        {
            StringBuilder sb = new StringBuilder();
            if (checkPoints.Count > 0)
            {
                sb.Append("[");
                string preQaEventType = string.Empty;
                string preMsgDictMessage = string.Empty;
                foreach (var item in checkPoints)
                {
                    if (item.QaEventType != preQaEventType)
                    {
                        preQaEventType = item.QaEventType;
                        sb.Append("{\"id\":\"" + item.QaEventType + "\",\"text\":\"" + item.QaEventType + "\"");
                        var msgdict = checkPoints.FindAll(m => m.QaEventType == item.QaEventType).OrderBy(m=>m.OrderValue).ToList();
                        if (msgdict.Count>0)
                        {
                            sb.Append(",\"children\":");
                            sb.Append("[");
                            foreach (var item1 in msgdict)
                            {
                                if (item1.MsgDictMessage != preMsgDictMessage)
                                {
                                    sb.Append("{\"id\":\"" + item1.MsgDictCode + "\",\"text\":\"" + item1.MsgDictMessage + "\"");

                                    //添加attributes
                                    sb.Append(",\"attributes\":");
                                    var pointcontent = checkPoints.FindAll(m => m.MsgDictCode == item1.MsgDictCode).OrderBy(m => m.OrderValue).ToList();
                                    sb.Append("[");

                                    foreach (var item2 in pointcontent)
                                    {
                                        sb.Append("{\"CheckPointID\":\"" + item2.CheckPointID + "\",\"OrderValue\":\"" + item2.OrderValue + "\",\"CheckPointContent\":\"" + item2.CheckPointContent + "\"},");
                                    }
                                    sb = sb.Remove(sb.Length - 1, 1);
                                    sb.Append("]");
                                ...
                                preMsgDictMessage = item1.MsgDictMessage;
                            }
                            sb = sb.Remove(sb.Length - 1, 1);
                            sb.Append("]");
                        }
        ...
        /// <param name="szCheckItems">选中的ID号集合</param>
        /// <returns></returns>
        public static string GetMenuJson(List<SysMenu> menus, int pId)
        {
