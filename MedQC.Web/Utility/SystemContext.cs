...
namespace MedQC.Web
{
    ...
    public class SystemContext
    {
        ...
        public List<Models.Advert> Adverts
        {
            get
            {
                if (CacheHelper.IsExistCache("advert"))
                {
                    this.m_adverts = CacheHelper.GetCache("advert") as List<Models.Advert>;
                }
                else
                {
                    this.m_adverts = EnterRepository.GetRepositoryEnter().AdvertRepository.GetAdverts(null, null).ToList();
                    CacheHelper.AddCache("advert", this.m_adverts, 1);
                }
                return m_adverts;
            }
        }

        private Dictionary<string,string> m_opreationParameter= null;
        public Dictionary<string,string> OperationParameter
        {
            get
            {
                if (CacheHelper.IsExistCache("OperationParameter"))
                {
                    this.m_opreationParameter = CacheHelper.GetCache("OperationParameter") as Dictionary<string,string>;
                }
                else
                {
                    this.m_opreationParameter = new Dictionary<string, string>();
                    var result = IBatisAccess.SysParameterDao.Instance.GetChildNodesByCode(SystemConst.OperationParameter.operation);
                    foreach (var item in result)
                    {
                        if (this.m_opreationParameter.ContainsKey(item.Code))
                            continue;
                        this.m_opreationParameter.Add(item.Code, item.Value);
                    }
                    CacheHelper.AddCache("OperationParameter", this.m_opreationParameter, 1);
                }
                return m_opreationParameter;
            }
        }
        ...
        {
            return "/Content/images/site/defaultpic.gif";
        }
        /// <summary>
        /// 默认密码 六个1，得到考生登陆默认密码
        /// </summary>
