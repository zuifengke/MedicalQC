...
namespace MedQC.Web
{
    public class WebCookieHelper
    {
        ...
        public const string SysUserCookieName = "SysUserCookie";
        /// <summary>
        /// 设置系统用户信息
        /// </summary>
        public static void SetSysUserCookie(int id, string name, string LoginName, string tel, int day)
        {
            string cookieValue = string.Join("|*&^%$#@!", id, HttpUtility.UrlEncode(name, System.Text.Encoding.UTF8), HttpUtility.UrlEncode(LoginName, System.Text.Encoding.UTF8), tel);
            if (day == 0)
            {
                Utility.CookieHelper.SetCookie(SysUserCookieName, cookieValue, "");
            }
            else
            {
                Utility.CookieHelper.SetCookie(SysUserCookieName, cookieValue, "", day);
            }
        }
        /// <summary>
        /// 获取系统用户基本信息
        /// </summary>
        /// <param name="index">【0：id，1：name，2:LoginName,3:tel</param>
        /// <returns></returns>
        public static string GetSysUserInfo(int index)
        {
            string value = string.Empty;
            string cookieValue = Utility.CookieHelper.GetCookie(SysUserCookieName);
            if (!string.IsNullOrEmpty(cookieValue))
            {
                string[] info = cookieValue.Split(new string[] { "|*&^%$#@!" }, StringSplitOptions.None);
                if (info.Length >= index)
        ...
        {
            string id = GetSysUserInfo((int)SysUserInfo.ID);
            return string.IsNullOrEmpty(id) ? 0 : int.Parse(id);
        }
        /// <summary>
        /// 判断系统用户是否登陆
        /// </summary>
        /// <returns></returns>
        public static bool SysUserCheckLogin()
        {
            if (Utility.CookieHelper.ExistCookie(SysUserCookieName))
            {
                return true;
            }
            else
        ...
        /// 注销系统用户Cookie
        /// </summary>
        public static void SysUserLoginOut()
        {
            if (Utility.CookieHelper.ExistCookie(SysUserCookieName))
            {
                Utility.CookieHelper.ExpireCookie(SysUserCookieName);
            }
        }
        #endregion
    }
